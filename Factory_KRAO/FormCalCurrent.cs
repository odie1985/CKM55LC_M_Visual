...
{
    public partial class FormCalCurrent : Form
    {
        /// <summary>
        /// 日志
        /// </summary>
        static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(FormCalCurrent));
        ...
        private void btnStep1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                sumA += ck.CalibrationValues.Smp_CurrentA;
                sumB += ck.CalibrationValues.Smp_CurrentB;
                sumC += ck.CalibrationValues.Smp_CurrentC;
            }
        ...
        private void setKB()
        {
            ...
            if (chkA.Checked)
            {
                ck.CalibrationValues.CurrentA_K = (short)A.k;
                ck.CalibrationValues.CurrentA_B = (short)A.b;
            }

            if (chkB.Checked)
            {
                ck.CalibrationValues.CurrentB_K = (short)B.k;
                ck.CalibrationValues.CurrentB_B = (short)B.b;
            }
            
            if (chkC.Checked)
            {
                ck.CalibrationValues.CurrentC_K = (short)C.k;
                ck.CalibrationValues.CurrentC_B = (short)C.b;
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                if (chkA.Checked)
                {
                    if (ck.CalibrationValues.CurrentA_K != (short)A.k ||
                        ck.CalibrationValues.CurrentA_B != (short)A.b ||
                        ck.IsOffline == true)
                    {
                        err = 1;
                ...
                if (chkB.Checked)
                {
                    if (ck.CalibrationValues.CurrentB_K != (short)B.k ||
                        ck.CalibrationValues.CurrentB_B != (short)B.b ||
                        ck.IsOffline == true)
                    {
                        err = 1;
                ...
                if (chkC.Checked)
                {
                    if (ck.CalibrationValues.CurrentC_K != (short)C.k ||
                        ck.CalibrationValues.CurrentC_B != (short)C.b ||
                        ck.IsOffline == true)
                    {
                        err = 1;
        ...
        {
            if (Math.Abs(f.x2 - f.x1) <= 50)
            {
                throw new Exception("错误：两次采样值差小于50！");
            }

            f.k = (f.y2 - f.y1) / (f.x2 - f.x1);
            f.b = f.y2 - f.k * f.x2;

            f.k = Math.Round(f.k * 1024);
            f.b = Math.Round(f.b * 1024);
        }
